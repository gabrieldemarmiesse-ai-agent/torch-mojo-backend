...
same-context FIFO ordering preserves dependencies. `entry.mojo` (the family's
`tmb_call`) is new and combines the three backward launches. The files have
since been formatted with `mojo format` and renamed into this package
(`fwd_kernel.mojo`, `bwd_launch.mojo`, ...), so the hashes above identify the
upstream sources, not the files here.
