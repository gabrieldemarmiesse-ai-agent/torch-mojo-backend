# ===----------------------------------------------------------------------=== #
# C entry of the vendored dense FA4 kernels (family `fa4`).
#
...
# wgmma emitter, selected inside the kernel, see `wgmma_f16.mojo`).
# The family is `tmb/kernels/fa4`, found like every other one through
# `Loader.family_dir`; its closure is its own files, so a FlashAttention
# change rehashes nothing else.
#
...
from tmb.kernels.fa4.bwd_launch import (
    launch_bwd_convert,
    launch_bwd_main,
    launch_bwd_preprocess,
)
from tmb.kernels.fa4.fwd_launch import launch_fwd_fa4
from tmb.kernels.fa4.fwd_selfload_common import kFa4BlockM as kFa4SelfloadBlockM
from tmb.kernels.fa4.fwd_selfload_common import (
    kFa4CtasPerSm as kFa4SelfloadCtasPerSm,
)
...
# tmb/ops/attention.mojo gates on the same conditions; these are the defensive
