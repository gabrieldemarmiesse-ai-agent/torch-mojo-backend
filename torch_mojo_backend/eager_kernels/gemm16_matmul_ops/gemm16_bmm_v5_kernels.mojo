...
# the instantiation, of which 36 of the 40 exceed 49152 bytes.  ptxas caps a
# kernel's *static* `.shared` at exactly that on sm_90 up to CUDA 12.8 and
# fails the whole `mojo build` -- not just the offending kernel -- so where the
# active assembler is an older one those slabs have to come from the dynamic
# (`extern`) shared window instead, which has never been subject to the cap
# (it is opted into per launch with MAX_DYNAMIC_SHARED_SIZE_BYTES, the scheme
# eager_flash_attention has always used).
#
# `_big_static_smem_on()` says which assembler the loader probed (see
# variant_gates.mojo).  When it is true the allocations below are compiled
# exactly as they were before this dual mode existed, byte for byte; when it
# is false the instantiations that DO NOT FIT carve one `external_memory` slab
# into the same three tiles in the same order.  The ones that already fit --
# `tiny_m64n64` at 40976 B and `tiny_m64n64_scalar_c` at 32784 B, the two the
# audit passed -- stay static in every build, because the dynamic window is not
...
# cost the smallest one a resident CTA.  Converting only what must convert also
# keeps those two bit-identical across both regimes.
#
# The mbarriers stay static in both modes: 16-48 bytes never threatened the
# cap, and keeping them out of the carve keeps the dynamic size a simple
# function of the tile geometry.  The scalar-C instantiations reserve 1 KiB for
# the dummy C tile the static build dead-eliminates, because a carve that
# dropped it would hand the epilogue a pointer into the B pipeline if anyone
# ever un-gated it; no converted config loses a resident CTA to that 1 KiB.
#
...
comptime _B5_STATIC_SMEM_CAP = 49152
...
def _b5_smem_bytes[stages: Int, bm: Int, bn: Int, tma_store: Bool]() -> Int:
    ...
    the static allocations are declared in, so the two modes lay the tiles out
    identically.  Zero when the tiles are static: the dynamic window is unused
    then and the launch must not reserve any of it.
    """
...
def _b5_bmm_nn_persistent_ws[
    ...
    comptime if _is_sm_9x():
        ...
        # A/B pipeline + C tile, static or dynamic (see the header comment).
        var a_smem: _B5_SMEM
        var b_smem: _B5_SMEM
        var c_smem: _B5_SMEM
        comptime if not _b5_dynamic_smem[stages, bm, bn, tma_store]():
...
def _b5_enqueue_tiny[
    ...
    # Both are the launch defaults (absent) in the static-smem mode.
